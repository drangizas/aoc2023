...
fn main() {
    ...
    let seeds_raw = lines.get(0).expect("File is malformed (no first line)")[7..]
        .split_ascii_whitespace()
        .filter_map(|digit| digit.parse::<usize>().ok())
        .collect::<Vec<usize>>();
    let mut idx = 0_usize;
    while idx < seeds_raw.len() {
        seeds.push((seeds_raw[idx], seeds_raw[idx + 1]));
    ...
    for (seed_begin, seed_range) in seeds {
        // Calculate the total effect of mappings on the entire range
    }
